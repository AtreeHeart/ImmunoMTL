...
# Load HLA clusters (all MHCs to be randomized here)
hla_clusters = pd.read_csv("../HLA/clustering_res.csv")
original_mapping = hla_clusters.set_index("HLA")["cluster"].to_dict()
original_cluster_sizes = Counter(original_mapping.values())

all_hlas = hla_clusters["HLA"].unique().tolist()
original_labels = [original_mapping[hla] for hla in all_hlas]

# Random assignment across all MHCs in clustering_res
random.seed(1)
shuffled_hlas = all_hlas.copy()
random.shuffle(shuffled_hlas)
mhc_to_random_cluster = {}
start = 0
for cluster_id, size in original_cluster_sizes.items():
    selected = shuffled_hlas[start:start+size]
    for hla in selected:
        mhc_to_random_cluster[hla] = cluster_id
    start += size

random_labels = [mhc_to_random_cluster[hla] for hla in all_hlas]
cluster_ids = sorted(set(mhc_to_random_cluster.values()))

# === Compare the two clusterings ===
ari = adjusted_rand_score(original_labels, random_labels)
nmi = normalized_mutual_info_score(original_labels, random_labels)

print(f"[INFO] Adjusted Rand Index (ARI): {ari:.4f}")
print(f"[INFO] Normalized Mutual Information (NMI): {nmi:.4f}")

# Save mapping and IDs
pd.DataFrame(list(mhc_to_random_cluster.items()), columns=["HLA", "cluster"]).to_csv("../HLA/random_mhc_cluster_assignment_seed1.csv", index=False)
import joblib
joblib.dump(cluster_ids, "../models/random_cluster_ids1.pkl")

# Apply to training data
training["cluster"] = training["MHC"].map(mhc_to_random_cluster)
training = training.dropna(subset=["cluster"])
...
def train_mtl_model(df, seed=42, save_path="mtl_unified_model.pt"):
...
training, 
seed=44, 
save_path=f"../models/MTL_model_shuffle_r44.pt"
)
