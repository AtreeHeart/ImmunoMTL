...
cluster_mapping = pd.read_csv("../HLA/random_mhc_cluster_assignment_seed1.csv").set_index("HLA")["cluster"].to_dict()
cluster_ids = joblib.load("../models/random_cluster_ids1.pkl")
...
model.load_state_dict(torch.load("../models/ImmunoMTL_shuffle_r44.pt", map_location="cuda"))  # or "cuda"

def evaluate_dataset(model, dataset, mhc="MHC", pep="Peptide", label="Label", name="Dataset"):
    model.eval()
    ...
    results_df = pd.DataFrame(columns=["Peptide", "MHC", "MHC Cluster", "Label", "Predicted Score"])
    ...
    for cluster in sorted(dataset["cluster"].unique()):
        ...
        x_pep = torch.tensor(peptide_emb[indices], dtype=torch.float32)
        x_mhc = torch.tensor(mhc_emb[indices], dtype=torch.float32)
        ...
        cluster_results = pd.DataFrame({
            ...
            "MHC Cluster": cluster_data["cluster"].values,
            "Label": y_true,
            "Predicted Score": preds
        })
        results_df = pd.concat([results_df, cluster_results], ignore_index=True)
    os.system("mkdir -p ../pred_results/immunomtl_shuffle")
    results_df.to_csv(f"../pred_results/immunomtl_shuffle/{name}.csv", index=False)
