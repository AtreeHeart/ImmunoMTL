#!/usr/bin/env python3
"""
eval_STL.py — Inference-only evaluation for STL ablation model (seed=22).

Architecture: ESM2-t12-35M (frozen) → dual BiLSTM (pep, MHC separate, 480→128)
              → shared FC(256→64→16) → single Linear(16,1) head, sigmoid outside.
Matches train_STL.py exactly.

Outputs:
  pred_results/finetune/STL_s22_aA_pw4.0_ep100_el2.0_wd1e-4_wSparse_hnC0-1/{benchmark,mRNA,zero1,zero2}_predictions.csv
"""
import os
import numpy as np
import pandas as pd
...
# ── Config (must match ImmunoSTL_retrain.py) ──────────────────────────────────
ESM_ID  = "facebook/esm2_t12_35M_UR50D"
ESM_DIM = 480
PEP_LEN = 11
MHC_LEN = 34
CKPT    = "../models/ImmunoSTL_s22.pt"
PRED_DIR = "../pred_results/STL"
DATA_DIR = "../data"
HLA_DIR  = "../HLA"
...
class STLModel(nn.Module):
    def __init__(self):
        ...
        self.heads = nn.ModuleList([nn.Linear(16, 1)])
    def forward(self, xp, xm):
        p, _ = self.pep1(xp); p, _ = self.pep2(p); p = p[:, -1, :]
        m, _ = self.mhc1(xm); m, _ = self.mhc2(m); m = m[:, -1, :]
        return self.heads[0](self.shared(torch.cat([p, m], dim=1))).squeeze(-1)
...
def predict(df):
    ...
    me = torch.tensor(np.stack([MHC_EMB[m] for m in df["HLA_pseudo"]]), dtype=torch.float32)
    out = []
    for i in range(0, len(pe), 512):
        with torch.no_grad():
            out.append(torch.sigmoid(model(pe[i:i+512].to(device), me[i:i+512].to(device))).cpu().numpy())
